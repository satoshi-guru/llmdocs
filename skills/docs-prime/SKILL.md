---
name: docs-prime
description: Full pre-sprint doc workflow in one command — fetch library docs → index COMPACT.md → compile LIB-CONTEXT.md (wraps /docs-fetch → /docs-distill → /docs-context). Use when the user is about to start coding — a sprint, refactor, or feature ("before we touch any code") — and wants the docs for the libraries they'll use prepped and ready first. Triggers: "prime the docs for X and Y", "prep doc context", "get docs ready before the refactor", "run the full doc workflow", "starting the X sprint tomorrow, prime all the docs I'll need: A, B, C". Usually names 2+ libraries or a named stack (expo+supabase, fastapi+pydantic, python+hyperliquid). Don't use for a quick one-off single-library grab (use /docs-fetch), or to only re-compile (/docs-context) or re-index (/docs-distill) docs already in the store.
argument-hint: "[lib1] [lib2] ... — library names/URLs to fetch. Omit to re-run docs-distill + docs-context on existing docs only."
allowed-tools: Bash Read Write
---

# docs-prime — Library Doc Workflow
...
compiling because `docs-context` is a *consumer* of `COMPACT.md`, not a parallel generator.

Arguments: $ARGUMENTS

---

## Step 1 — Fetch docs (docs-fetch)

**Follow the `docs-fetch` skill instructions** for `$ARGUMENTS` — do **not** call `crawler.py`
directly here. `/docs-fetch` is the single owner of token resolution (engine preset → `preset:`
group → alias → raw URL) and picks `--preset` vs `--url` correctly. This matters: routing a
React-SPA preset like `discord` through a plain `--url` fetches 0 pages, so docs-prime must not
re-implement the fetch — it would drift from docs-fetch and miss the strategy routing. (Steps 2–3
below already defer to the docs-context / docs-distill skills the same way; Step 1 is just
consistent with them.)

`/docs-fetch` writes to the global store `~/.llmdocs/docs/<slug>/`, runs fetches sequentially with
...
**Important contract with Step 3:** `/docs-fetch` already auto-chains `/docs-distill` after a
...
## Step 2 — Index (docs-distill)

Index **before** compiling — the indexer produces the `COMPACT.md` + `LOOKUP.md` tiers that
`docs-context` (Step 3) consumes. Libs fetched in Step 1 are **already indexed** (`/docs-fetch`
auto-chains `/docs-distill` after a successful fetch), so only index slugs that still lack a
...
# Exclude libs fetched in Step 1 — /docs-fetch already ran docs-distill for those.
# FETCHED_SLUGS should be set to the space-separated list of slugs from Step 1.
for d in ~/.llmdocs/docs/*/; do
  slug=$(basename "$d")
  # Skip if this slug was fetched in the current run (already indexed by /docs-fetch)
  case " $FETCHED_SLUGS " in *" $slug "*) continue ;; esac
  [ -f "$d/COMPACT.md" ] || echo "needs index: $slug"
done
```

For any that need it, **follow the `docs-distill` skill instructions** (it reads
...
## Step 3 — Compile context (docs-context)

Now compile `LIB-CONTEXT.md`. `docs-context` is a **consumer of COMPACT.md** — it reads the
...
for slug in $FETCHED_SLUGS; do
  ...
  test -f "$f" && echo "OK: $f" || echo "MISSING: $f — docs-distill may have failed"
done
```

Then follow the docs-context skill instructions fully to produce `LIB-CONTEXT.md`.
